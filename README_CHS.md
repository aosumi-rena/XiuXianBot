...
- **utils/**  
  一些辅助执行某些命令的副功能逻辑代码
    - `account_status.py` 负责帮助机器人获取用户数据/状态，并生成markdown格式的数据文本
    - `database.py` 负责备份用户数据、生成通用uid、保证默认数值（避免出错）
    - `localisation.py` 负责让机器人获取本地化的文本并发送给用户
    - `otp.py` 负责生成连接账号的一次性验证码
- **textmaps/**  
...
### 网页界面
- **本地网页管理面板** (`web_local/`)  
  基于 Flask 的一个本地网页版管理面板，默认运行在`http://localhost:11451`，提供单页标签式界面：
...
运行后，将：
* 在`http://localhost:11451`（预设端口，可在`config.json`中更改所选端口）打开机器人控制台网页
